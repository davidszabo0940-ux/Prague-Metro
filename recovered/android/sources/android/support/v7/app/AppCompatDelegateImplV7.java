...
class AppCompatDelegateImplV7 extends AppCompatDelegateImplBase implements MenuBuilder.Callback, LayoutInflaterFactory {
    ...
    private ViewGroup createSubDecor() {
        ...
        TypedArray a = this.mContext.obtainStyledAttributes(C0178R.styleable.Theme);
        if (!a.hasValue(C0178R.styleable.Theme_windowActionBar)) {
            a.recycle();
            throw new IllegalStateException("You need to use a Theme.AppCompat theme (or descendant) with this activity.");
        }
        if (a.getBoolean(C0178R.styleable.Theme_windowNoTitle, false)) {
            requestWindowFeature(1);
        } else if (a.getBoolean(C0178R.styleable.Theme_windowActionBar, false)) {
            requestWindowFeature(108);
        }
        if (a.getBoolean(C0178R.styleable.Theme_windowActionBarOverlay, false)) {
            requestWindowFeature(109);
        }
        if (a.getBoolean(C0178R.styleable.Theme_windowActionModeOverlay, false)) {
            requestWindowFeature(10);
        }
        this.mIsFloating = a.getBoolean(C0178R.styleable.Theme_android_windowIsFloating, false);
        a.recycle();
        LayoutInflater inflater = LayoutInflater.from(this.mContext);
        ViewGroup subDecor = null;
        if (!this.mWindowNoTitle) {
            if (this.mIsFloating) {
                subDecor = (ViewGroup) inflater.inflate(C0178R.layout.abc_dialog_title_material, (ViewGroup) null);
                this.mOverlayActionBar = false;
                this.mHasActionBar = false;
            } else if (this.mHasActionBar) {
                TypedValue outValue = new TypedValue();
                this.mContext.getTheme().resolveAttribute(C0178R.attr.actionBarTheme, outValue, true);
                if (outValue.resourceId != 0) {
                    themedContext = new ContextThemeWrapper(this.mContext, outValue.resourceId);
                } else {
                    themedContext = this.mContext;
                }
                subDecor = (ViewGroup) LayoutInflater.from(themedContext).inflate(C0178R.layout.abc_screen_toolbar, (ViewGroup) null);
                this.mDecorContentParent = (DecorContentParent) subDecor.findViewById(C0178R.id.decor_content_parent);
            ...
            if (this.mOverlayActionMode) {
                subDecor = (ViewGroup) inflater.inflate(C0178R.layout.abc_screen_simple_overlay_action_mode, (ViewGroup) null);
            } else {
                subDecor = (ViewGroup) inflater.inflate(C0178R.layout.abc_screen_simple, (ViewGroup) null);
            }
        ...
        if (this.mDecorContentParent == null) {
            this.mTitleView = (TextView) subDecor.findViewById(C0178R.id.title);
        }
        ViewUtils.makeOptionalFitsSystemWindows(subDecor);
        ViewGroup decorContent = (ViewGroup) this.mWindow.findViewById(R.id.content);
        ContentFrameLayout abcContent = (ContentFrameLayout) subDecor.findViewById(C0178R.id.action_bar_activity_content);
    ...
    private void applyFixedSizeWindow() {
        ...
        TypedArray a = this.mContext.obtainStyledAttributes(C0178R.styleable.Theme);
        a.getValue(C0178R.styleable.Theme_windowMinWidthMajor, cfl.getMinWidthMajor());
        a.getValue(C0178R.styleable.Theme_windowMinWidthMinor, cfl.getMinWidthMinor());
        if (a.hasValue(C0178R.styleable.Theme_windowFixedWidthMajor)) {
            a.getValue(C0178R.styleable.Theme_windowFixedWidthMajor, cfl.getFixedWidthMajor());
        }
        if (a.hasValue(C0178R.styleable.Theme_windowFixedWidthMinor)) {
            a.getValue(C0178R.styleable.Theme_windowFixedWidthMinor, cfl.getFixedWidthMinor());
        }
        if (a.hasValue(C0178R.styleable.Theme_windowFixedHeightMajor)) {
            a.getValue(C0178R.styleable.Theme_windowFixedHeightMajor, cfl.getFixedHeightMajor());
        }
        if (a.hasValue(C0178R.styleable.Theme_windowFixedHeightMinor)) {
            a.getValue(C0178R.styleable.Theme_windowFixedHeightMinor, cfl.getFixedHeightMinor());
        }
        a.recycle();
        cfl.requestLayout();
    }
        ...
        if (mode != null) {
            ...
            if (this.mActionModeView == null) {
                if (this.mIsFloating) {
                    ...
                    baseTheme.resolveAttribute(C0178R.attr.actionBarTheme, outValue, true);
                    if (outValue.resourceId != 0) {
                    ...
                    this.mActionModePopup = new PopupWindow(actionBarContext, (AttributeSet) null, C0178R.attr.actionModePopupWindowStyle);
                    PopupWindowCompat.setWindowLayoutType(this.mActionModePopup, 2);
                    this.mActionModePopup.setContentView(this.mActionModeView);
                    this.mActionModePopup.setWidth(-1);
                    actionBarContext.getTheme().resolveAttribute(C0178R.attr.actionBarSize, outValue, true);
                    ...
                    ViewStubCompat stub = (ViewStubCompat) this.mSubDecor.findViewById(C0178R.id.action_mode_bar_stub);
                    if (stub != null) {
    ...
    @Override // android.support.v7.app.AppCompatDelegate
    public View createView(View view, String name, @NonNull Context context, @NonNull AttributeSet attrs) {
        boolean isPre21 = Build.VERSION.SDK_INT < 21;
        if (this.mAppCompatViewInflater == null) {
            this.mAppCompatViewInflater = new AppCompatViewInflater();
        }
        boolean inheritContext = isPre21 && this.mSubDecorInstalled && shouldInheritContext((ViewParent) view);
        return this.mAppCompatViewInflater.createView(view, name, context, attrs, inheritContext, isPre21, true);
    }
    ...
    private void openPanel(PanelFeatureState st, KeyEvent event) {
        ...
        if (!st.isOpen && !isDestroyed()) {
            ...
            if (wm != null && preparePanel(st, event)) {
                ...
                st.isHandled = false;
                WindowManager.LayoutParams lp3 = new WindowManager.LayoutParams(width, -2, st.f12x, st.f13y, 1002, 8519680, -3);
    ...
    private boolean initializePanelMenu(PanelFeatureState st) {
        ...
        if ((st.featureId == 0 || st.featureId == 108) && this.mDecorContentParent != null) {
            ...
            if (outValue.resourceId != 0) {
                ...
                widgetTheme.resolveAttribute(C0178R.attr.actionBarWidgetTheme, outValue, true);
            } else {
                baseTheme.resolveAttribute(C0178R.attr.actionBarWidgetTheme, outValue, true);
            }
            if (outValue.resourceId != 0) {
                if (widgetTheme == null) {
                    widgetTheme = context.getResources().newTheme();
            ...
            if (this.mActionModeView.isShown()) {
                ...
                if (mlp.topMargin != newMargin) {
                    ...
                    if (this.mStatusGuard == null) {
                        ...
                        this.mStatusGuard.setBackgroundColor(this.mContext.getResources().getColor(C0178R.color.abc_input_method_navigation_guard));
    ...
    private static final class PanelFeatureState {
        ...
        void setStyle(Context context) {
            ...
            widgetTheme.resolveAttribute(C0178R.attr.actionBarPopupTheme, outValue, true);
            if (outValue.resourceId != 0) {
                widgetTheme.applyStyle(outValue.resourceId, true);
            }
            widgetTheme.resolveAttribute(C0178R.attr.panelMenuListTheme, outValue, true);
            if (outValue.resourceId != 0) {
                widgetTheme.applyStyle(outValue.resourceId, true);
            } else {
                widgetTheme.applyStyle(C0178R.style.Theme_AppCompat_CompactMenu, true);
            }
            Context context2 = new ContextThemeWrapper(context, 0);
            context2.getTheme().setTo(widgetTheme);
            this.listPresenterContext = context2;
            TypedArray a = context2.obtainStyledAttributes(C0178R.styleable.Theme);
            this.background = a.getResourceId(C0178R.styleable.Theme_panelBackground, 0);
            this.windowAnimations = a.getResourceId(C0178R.styleable.Theme_android_windowAnimationStyle, 0);
            a.recycle();
        }
        ...
        MenuView getListMenuView(MenuPresenter.Callback cb) {
            ...
            if (this.listMenuPresenter == null) {
                this.listMenuPresenter = new ListMenuPresenter(this.listPresenterContext, C0178R.layout.abc_list_menu_item_layout);
